...
std::tuple<torch::Tensor, torch::Tensor> volume_render_cuvol_backward(
    SparseGridSpec &, RaysSpec &, RenderOptions &, Tensor, Tensor);
...
std::tuple<torch::Tensor, torch::Tensor> volume_render_cuvol_image_backward(
    SparseGridSpec &, CameraSpec &, RenderOptions &, Tensor, Tensor);

// Returns MSE
Tensor volume_render_multistage_fused(SparseGridSpec &, RaysSpec &,
...
#define _REG_FUNC(funname) m.def(#funname, &funname)
    ...
    _REG_FUNC(volume_render_multistage_fused);
    _REG_FUNC(dilate);
    _REG_FUNC(tv);
    _REG_FUNC(tv_grad);
