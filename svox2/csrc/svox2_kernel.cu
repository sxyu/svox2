...
__global__ void sample_grid_sh_kernel(
    ...
    CUDA_GET_THREAD_ID(tid, points.size(0) * grid.sh_data.size(1));
    const int idx = tid % grid.sh_data.size(1);
    const int pid = tid / grid.sh_data.size(1);
...
#define MAYBE_READ_LINK(u) ((link_ptr[u] >= 0) ? grid.sh_data[link_ptr[u]][idx] : 0.f)
...
#define MAYBE_ADD_GRAD_LINK_PTR_D(u, content) if (link_ptr[u] >= 0)  \
            atomicAdd(&grad_data[link_ptr[u]][0], content)
